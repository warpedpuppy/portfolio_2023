...
export default function BouncePigCode (canvas) {
    return {
        ...
        start: function () {
			...
			const app = new PIXI.Application({ width: '1000px', height: '1000px', background: '#1099bb', resizeTo: window });
			...
			PIXI.Assets.load("/images/bouncePig/bpb.json")
			.then( sheet => {

				console.log(sheet)
				// let hero = new Hero(sheet);
				// hero.init();

				console.log(sheet.textures['heroFalling0001.png'])

				let arr = [
					sheet.textures['heroFalling0001.png'],
					sheet.textures['heroFalling0002.png']
				]
				
				const animatedSprite = new PIXI.AnimatedSprite(arr);
